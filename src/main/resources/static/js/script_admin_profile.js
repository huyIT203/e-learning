...
async function uploadAvatar(file) {
    ...
    try {
        const response = await fetch('/admin/api/profile/avatar', {
            method: 'POST',
            body: formData
        });
...
function initializeFormHandlers() {
    ...
    if (personalForm) {
        personalForm.addEventListener('submit', function(e) {
            e.preventDefault();
            handleFormSubmit(this, '/admin/api/profile/personal');
        });
    }
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function(e) {
            e.preventDefault();
            handleFormSubmit(this, '/admin/api/profile/contact');
        });
    }
...
function initializeNotifications() {
    ...
    toggles.forEach(toggle => {
        toggle.addEventListener('change', function() {
            ...
            saveSystemSetting(setting, enabled);
        });
    });
}

// Save System Setting
async function saveSystemSetting(setting, enabled) {
    try {
        ...
        if (result.success) {
            showNotification(`Cài đặt ${setting} đã được cập nhật`, 'success');
        } else {
            showNotification(result.message || 'Cập nhật cài đặt thất bại', 'error');
        }
    } catch (error) {
...
async function backupSystem() {
    if (!confirm('Bạn có chắc chắn muốn sao lưu hệ thống không?')) {
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            showNotification('Sao lưu hệ thống thành công', 'success');
        } else {
            showNotification(result.message || 'Sao lưu hệ thống thất bại', 'error');
        }
    } catch (error) {
        console.error('Error backing up system:', error);
        showNotification('Có lỗi xảy ra khi sao lưu hệ thống', 'error');
    }
}

async function clearLogs() {
    if (!confirm('Bạn có chắc chắn muốn xóa tất cả logs không? Hành động này không thể hoàn tác.')) {
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            showNotification(result.message || 'Xóa logs thất bại', 'error');
        }
    } catch (error) {
...
async function loadSystemStats() {
    try {
        ...
        if (result.success) {
            ...
            updateCounter('systemUptime', stats.systemUptime);
            
            // Update overview cards
            updateCounter('activeUsers', stats.activeUsers);
            updateCounter('newCourses', stats.newCourses);
