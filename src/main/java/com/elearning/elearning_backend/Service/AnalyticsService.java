...
public class AnalyticsService {
    ...
    public UserAnalyticsResponse getUserAnalytics(String userId) {
        ...
        var submissions = examSubmissionRepository.findByStudentId(userId);
        int examCount = submissions.size();
        double avgScore = submissions.stream()
                .mapToDouble(ExamSubmission::getScore)
                .average()
                .orElse(0);

        return new UserAnalyticsResponse(courseCount, lessonsDone, examCount, avgScore);
    }
